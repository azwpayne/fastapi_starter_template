...
async def log_request(request: Request) -> RequestLogSchema:
    ...
    return RequestLogSchema(request_id=request_id, method=request.method, path=request.url.path,
                            query_params=dict(request.query_params),
                            client_ip=request.client.host if request.client else None,
                            user_agent=request.headers.get("user-agent"), request_body=request_body, )
...
async def log_response(response: Response, response_time: float) -> ResponseLogSchema:
    ...
    return ResponseLogSchema(request_id=request_id, status_code=response.status_code, response_time=response_time,
